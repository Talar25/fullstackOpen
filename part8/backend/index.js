...
const resolvers = {
  Query: {
    ...
    allBooks: (root, args) => {
      if (args.author && args.genre) {
        let filteredBooks = books.filter((b) => b.author === args.author)
  ...
  Author: {
    bookCount: (root) => {
      const findBooks = books.filter((b) => b.author === root.name)
      return findBooks.length
    },
  },
  Mutation: {
    addBook: (root, args) => {
      const isAuthor = authors.find((a) => a.name === args.author)
      const newId = uuid()
      if (!isAuthor) {
        const newAuthor = { name: args.author, id: newId }
        authors = [...authors, newAuthor]
        const newBook = { ...args, id: newId }
        books = [...books, newBook]
        return newBook
      }
    ...
    editAuthor: (root, args) => {
      ...
      const changedAuthor = { ...foundAuthor, born: args.born }
